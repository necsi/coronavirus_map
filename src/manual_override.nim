...
when isMainModule:
  ...
  # Reads all the csvs
  open(confirmed_csv, newStringStream(readFile(ConfirmedURL)), "confirmed.csv")
  open(recovered_csv, newStringStream(readFile(RecoveredURL)), "recovered.csv")
  open(deaths_csv, newStringStream(readFile(DeathURL)), "deaths.csv")
